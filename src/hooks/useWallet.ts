...
export function useWallet() {
  ...
  const { data: alchemyTokens, isLoading: isLoadingTokens, refetch: refetchTokens } = useQuery({
    queryKey: ['alchemyPortfolio', address],
    queryFn: async () => {
      if (!address) return [];
      ...
      try {
        ...
        const tokens = await AlchemyAPI.getCompletePortfolio(address);
        return tokens;
      } catch (error) {
