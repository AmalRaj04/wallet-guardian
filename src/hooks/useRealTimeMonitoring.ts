...
export function useRealTimeMonitoring() {
   ...
   */
  const startMonitoring = useCallback(async (tokens: Token[]) => {
    ...
    for (const token of tokens) {
      try {
        ...
        if (riskScore.overall > 55) {
          ...
          toast.error(`⚠️ ${riskScore.category.toUpperCase()} risk detected: ${token.symbol}`, {
            duration: 5000,
          });
        }
      } catch (error) {
   ...
   */
  const handleMempoolAlert = useCallback((mempoolAlert: MempoolAlert, tokens: Token[]) => {
    // Find affected token
    const affectedToken = tokens.find(t => 
      mempoolAlert.transaction.to?.toLowerCase() === t.address.toLowerCase() ||
      mempoolAlert.transaction.from?.toLowerCase() === t.address.toLowerCase()
    );
