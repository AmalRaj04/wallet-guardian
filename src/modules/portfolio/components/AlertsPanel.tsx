...
import { Bell, X, AlertTriangle, CheckCircle, Info, Shield, TrendingDown } from 'lucide-react';
...
export default function AlertsPanel() {
  const { alerts, markAlertAsRead, clearAlerts, threatsBlocked, fundsProtected } = useRealTimeMonitoring();
      ...
      <GlassCard className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-space-grotesk font-bold">Security Alerts</h2>
        ...
        {alerts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              No security alerts at the moment. We're monitoring your wallet 24/7.
            </p>
          </div>
        ) : (
                      ...
                      {alert.recommendation && (
                        <div className="bg-white/5 rounded p-3 mb-3">
                          <p className="text-sm text-gray-300">
                            <span className="font-semibold text-neon-blue">Recommendation:</span>{' '}
                            {alert.recommendation}
                          </p>
                        </div>
                      )}
                          ...
                          {alert.actions.map((action) => (
                            <button
                              ...
                              className={`px-3 py-1 text-sm rounded-lg transition-colors ${
                                action.type === 'convert' || action.type === 'revoke'
