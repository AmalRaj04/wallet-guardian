...
import { Shield, AlertTriangle, CheckCircle, Loader2, X, RefreshCw } from 'lucide-react';
...
export default function TokenAllowances({ tokens, walletAddress }: TokenAllowancesProps) {
  ...
  const fetchAllowances = async () => {
    ...
    try {
      ...
      const commonSpenders = [
        ...
        { address: '0x3fC91A3afd70395Cd496C647d5a6CC9D4B2b7FAD', name: 'Uniswap Universal Router 2' },
      ];

      for (const token of tokens.slice(0, 20)) { // Limit to first 20 tokens
        for (const spender of commonSpenders) {
          try {
            ...
            if (!allowanceHex || allowanceHex === '0x' || allowanceHex === '0x0') {
            ...
            if (allowanceValue > BigInt(0)) {
              const allowanceFormatted = Number(allowanceValue) / Math.pow(10, token.decimals);
            ...
            console.error(`Error fetching allowance for ${token.symbol} on ${spender.name}:`, error);
          }
        }
      }
  ...
  const revokeAllowance = async (allowance: TokenAllowance) => {
    ...
    try {
      ...
      toast.success(`Revoked approval for ${allowance.token.symbol}`, { id: 'revoke' });
      
      // Remove from list
      setAllowances(prev => prev.filter(a => 
        !(a.spender === allowance.spender && a.token.address === allowance.token.address)
      ));
      
    } catch (error: any) {
      console.error('Error revoking allowance:', error);
      toast.error(error.message || 'Failed to revoke approval', { id: 'revoke' });
    } finally {
      setRevoking(null);
    }
  };
  ...
  const selectAll = () => {
    if (selectedAllowances.size === allowances.length) {
      ...
      setSelectedAllowances(new Set(allowances.map(a => a.spender + a.token.address)));
    }
  };
  ...
  if (!walletAddress) {
    return (
      <GlassCard className="p-12 text-center">
        ...
        <p className="text-gray-400">Connect your wallet to view token approvals</p>
      </GlassCard>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <GlassCard className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">Manage your ERC-20 token allowances</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-red-500/10 rounded-lg p-4">
            <div className="text-2xl font-bold text-red-400">
              {allowances.filter(a => a.riskLevel === 'critical' || a.riskLevel === 'high').length}
            </div>
        ...
        <GlassCard className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              >
                {selectedAllowances.size === allowances.length ? 'Deselect All' : 'Select All'}
              </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">
              You don't have any active token approvals. This is good for security!
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold">{allowance.token.symbol}</div>
                            <div className="text-sm text-gray-400">{allowance.token.name}</div>
                          </div>
                        </div>
                        <span className={`px-3 py-1 rounded-full text-xs uppercase font-bold ${
                          allowance.riskLevel === 'critical' ? 'bg-red-500 text-white' :
                          allowance.riskLevel === 'high' ? 'bg-orange-500 text-white' :
                          allowance.riskLevel === 'medium' ? 'bg-yellow-500 text-black' :
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm mb-3">
                        <div>
                          ...
                          <span className="font-mono">{allowance.spenderName || allowance.spender.slice(0, 10) + '...'}</span>
                        </div>
                        <div>
                          <span className="text-gray-400">Allowance:</span>{' '}
                          <span className={allowance.isUnlimited ? 'text-red-400 font-bold' : ''}>
                            {allowance.isUnlimited ? 'UNLIMITED' : allowance.allowanceFormatted.toFixed(4)}
                          </span>
                        </div>
                      </div>
                      ...
                      {allowance.isUnlimited && (
                        <div className="bg-red-500/20 border border-red-500/30 rounded p-2 mb-3 text-sm">
                          ...
                          Unlimited approval allows the spender to transfer any amount of your tokens at any time
                        </div>
                      )}
                    </div>
      ...
      <GlassCard className="p-6 bg-blue-500/5 border-blue-500/30">
        ...
        <p className="text-sm text-gray-300 mb-2">
          Token approvals allow smart contracts to spend your tokens on your behalf. While necessary for DeFi interactions,
        ...
        <ul className="text-sm text-gray-300 space-y-1 ml-4">
          ...
          <li>• You no longer use the protocol but the approval remains active</li>
        </ul>
        <p className="text-sm text-gray-300 mt-2">
          <strong>Best Practice:</strong> Revoke approvals you no longer need and avoid unlimited approvals when possible.
        </p>
      </GlassCard>
    </div>
  );
