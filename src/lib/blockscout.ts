...
// Use our Next.js API route instead of calling Blockscout directly
const BLOCKSCOUT_BASE_URL = "/api/blockscout";
...
// API instance now points to our Next.js API route (server-side)
...
export class BlockscoutAPI {
  ...
  static async isContractVerified(contractAddress: string): Promise<boolean> {
    try {
      ...
      // Silently fail - contract verification is optional
      // Network errors are common with Blockscout
      if (error.code !== "ERR_NETWORK") {
        console.warn(
          "Blockscout API unavailable - skipping verification check"
        );
      }
      return false; // Assume not verified if we can't check
    }
  }
  ...
  static async getContractSource(contractAddress: string): Promise<any> {
    try {
      ...
      // Silently fail - contract source is optional
      if (error.code !== "ERR_NETWORK") {
        console.warn("Blockscout API unavailable - skipping source fetch");
      }
      return null; // Return null instead of throwing
    }
  }
  ...
  static async getTokenTransfers(
    ...
    try {
      ...
      // Silently fail - token transfers are optional
      if (error.code !== "ERR_NETWORK") {
        console.warn("Blockscout API unavailable - skipping token transfers");
      }
      return [];
    }
  }
  ...
  static async analyzeContractSecurity(
    contractAddress: string
        ...
        try {
          ...
          if (code.includes("selfdestruct") || code.includes("suicide")) {
            risks.push({
              ...
              recommendation:
                "Contract can be destroyed, potentially locking funds",
              timestamp: new Date(),
            });
          }
          ...
          if (code.includes("delegatecall")) {
            risks.push({
              ...
              recommendation:
                "Review delegatecall usage for potential vulnerabilities",
              timestamp: new Date(),
            });
          }
...
export function calculateRiskScore(
  ...
  const highRiskAllowances = allowances.filter(
    (a) => a.riskLevel === "high"
  ).length;
