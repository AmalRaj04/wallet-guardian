...
export default function AIAnalysis({ coin }: AIAnalysisProps) {
  ...
  const analyzeCoins = async () => {
    ...
    try {
      // Convert coin data to our Coin type for analysis
      const coinForAnalysis = {
        id: coin.id,
        symbol: coin.symbol,
        name: coin.name,
        ...
        ath_change_percentage: coin.market_data?.ath_change_percentage?.usd || 0,
        ath_date: coin.market_data?.ath_date?.usd || '',
        atl: coin.market_data?.atl?.usd || 0,
        atl_change_percentage: coin.market_data?.atl_change_percentage?.usd || 0,
        atl_date: coin.market_data?.atl_date?.usd || '',
        last_updated: coin.last_updated || new Date().toISOString(),
      };

      const aiAnalysis = await GroqAI.analyzeCoin(coinForAnalysis);
